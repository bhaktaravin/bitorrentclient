...
use peer::{block_length_for_piece, download_block, piece_size};
use torrent::TorrentMetadata;
use tracker::{generate_peer_id, AnnounceEvent, AnnounceRequest};
...
fn main() -> Result<()> {
    ...
    if !cli.no_announce {
        if let Some(announce_url) = &metadata.announce {
            ...
            println!("Tracker peers: {}", response.peers.len());
    ...
    if wants_download {
        ...
        return download::download_torrent(&metadata, &peer_list, &output_dir, peer_id);
    }
        ...
        match download_block(
            ...
            Ok(block) => {
                ...
                if block.data.is_ascii() {
                    ...
                    println!("Data (hex):   {}", hex::encode(&block.data));
                }
                return Ok(());
            }
            Err(err) => {
